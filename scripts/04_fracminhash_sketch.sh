...
#
# Outputs:
#   data/GTDB/fracminhash_sketches/{genome_id}.fracminhash.sketch  (one per genome)
#   data/GTDB/fracminhash_sketches/sketch_run_stats.json           (timing + disk usage)
#
# Prerequisites:
...
#   TEST_N=200 bash 04_fracminhash_sketch.sh
#
# Full run — sketch all genomes (unset or 0 means "all"):
#   bash 04_fracminhash_sketch.sh
# ==============================================================================
set -euo pipefail
...
# ---------------------------------------------------------------------------
# FracMinHash parameters — made explicit even where they equal defaults
# ---------------------------------------------------------------------------
ALGO="fracminhash"
KMER=31
SCALE=0.001     # scale = 1/scaled; 0.001 matches sourmash scaled=1000
SEED=42
...
# Override at the command line: TEST_N=500 bash 04_fracminhash_sketch.sh
# ---------------------------------------------------------------------------
TEST_N="${TEST_N:-0}"
...
while IFS= read -r genome_path; do
    ...
    out_sketch="${SKETCH_DIR}/${genome_name}.fracminhash.sketch"
...
N_SKETCHED=$(find "${SKETCH_DIR}" -maxdepth 1 -name "*.fracminhash.sketch" | wc -l)
...
SCALED=$(python3 -c "print(int(1/${SCALE}))")
...
import json
stats = {
    "script":                "04_fracminhash_sketch.sh",
    "algo":                  "${ALGO}",
    "kmer":                  ${KMER},
    "scale":                 ${SCALE},
    "scaled_equivalent":     ${SCALED},
    "seed":                  ${SEED},
    "canonical":             True,
...
printf "  Parameters        : algo=%s  k=%d  scale=%s (scaled=%s)  seed=%d\n" \
       "${ALGO}" "${KMER}" "${SCALE}" "${SCALED}" "${SEED}"
echo "========================================================================="
echo ""
echo "Next step:"
echo "  python3 ${BASE_DIR}/scripts/07_fracminhash_pairwise.py \\"
echo "      --sketch-dir ${SKETCH_DIR} \\"
echo "      --candidates ${GTDB_DIR}/gtdb_pairwise_containment.csv \\"
echo "      --output     ${GTDB_DIR}/fracminhash_pairwise \\"
echo "      --cores      192"
echo ""
echo "Or run the wrapper:"
echo "  bash ${BASE_DIR}/scripts/07_fracminhash_pairwise.sh"
