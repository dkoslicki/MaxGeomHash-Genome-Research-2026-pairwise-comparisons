...
sketching methods (AlphaMaxGeomHash, FracMinHash) against the KMC exact
k-mer ground truth.
...
truth.  Color encoding: red (0 = perfect estimate) → blue (high relative
error).  Missing pairs (below the 0.01 similarity threshold) are shown in
light gray.

To keep the heatmaps visually comparable:
  - Both panels share the same colormap and vmin/vmax.
...
---------------------------
Adding a future method (e.g., MinHash) requires only:
  1.  Writing a loader function (or re-using load_npz_sorted if it outputs
      an NPZ with the same format as AMG/KMC).
  2.  Adding one entry to the METHODS list in main() with:
        {"label": ..., "keys": ..., "metric_arrays": {...}}
  The loop that builds submatrices, computes errors, and draws panels is
  fully generic — it iterates over METHODS with no method-specific logic.

Usage:
    python3 06_plot_heatmaps.py \\
        --amg-pairwise  data/GTDB/alphamaxgeom_pairwise \\
        --fracminhash   data/GTDB/gtdb_pairwise_containment.csv \\
        --kmc-pairwise  data/GTDB/kmc_pairwise \\
        --output        data/GTDB/figures \\
        [--metric jaccard|max_containment|containment] \\
        [--n-genomes 500] \\
        [--show-reference]   # also draw the KMC ground-truth similarity panel
"""
...
def parse_args():
    ...
    p.add_argument("--amg-pairwise", required=True,
                   help="AMG pairwise directory (pairwise_results.npz + genome_index.json)")
...
def main():
    ...
    log.info("Loading AMG pairwise ...")
    amg = load_npz_sorted(
        Path(args.amg_pairwise) / "pairwise_results.npz",
        Path(args.amg_pairwise) / "genome_index.json",
    )
    # AMG uses the same genome index as KMC (written from the same manifest),
    # so their integer indices are identical — no remapping needed.

    log.info("Loading FracMinHash pairwise ...")
    fmh = load_fracminhash_sorted(
        Path(args.fracminhash),
    ...
    amg_sim = build_submatrix(amg["keys"], amg[metric], selected, N)
    fmh_sim = build_submatrix(fmh["keys"], fmh[metric], selected, N)
    ...
    # ------------------------------------------------------------------
    # Compute relative error matrices
    # ------------------------------------------------------------------
    amg_err = relative_error_matrix(amg_sim, kmc_sim)
    fmh_err = relative_error_matrix(fmh_sim, kmc_sim)

    # Summary statistics
    for name, err in [("AMG", amg_err), ("FMH", fmh_err)]:
        valid = err[~np.isnan(err)]
        if len(valid):
            log.info("  %s relative error: median=%.4f  99pct=%.4f  max=%.4f",
                     name, np.median(valid), np.percentile(valid, 99), valid.max())
    ...
    #  METHODS list — add a new entry here to include another algorithm.
    #  Each entry needs: label (panel title) and err (M×M relative error array).
    methods = [
        {"label": "AlphaMaxGeomHash\n(W=64, α=0.45, k=31)", "err": amg_err},
        {"label": "FracMinHash\n(scaled=1000, k=31)",        "err": fmh_err},
        # Future MinHash entry would go here:
        # {"label": "MinHash\n(num=1000, k=31)", "err": minhash_err},
    ]

    log.info("Generating heatmap figure (%s) ...", metric_label)
    plot_heatmaps(
        kmc_sim      = kmc_sim,
        methods      = methods,
