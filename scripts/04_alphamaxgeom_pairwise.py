...
Usage:
  # Test run (after TEST_N=200 bash 03_alphamaxgeom_sketch.sh):
  python3 04_alphamaxgeom_pairwise.py \\
      --sketch-dir /scratch/.../alphamaxgeom_sketches \\
      --candidates /scratch/.../gtdb_pairwise_containment.csv \\
      --output     /scratch/.../alphamaxgeom_pairwise \\
      --cores      192
...
def discover_sketches(sketch_dir: str) -> set:
    ...
    return {
        p.name.replace(".alphamaxgeom.sketch", "")
        for p in Path(sketch_dir).glob("*.alphamaxgeom.sketch")
    }
...
def _run_filter_for_query(args: tuple):
    ...
    query_sketch = os.path.join(sketch_dir, f"{query_id}.alphamaxgeom.sketch")
    if not os.path.exists(query_sketch):
        return None

    # Validate which neighbors actually have sketch files
    ref_paths = {
        nid: os.path.join(sketch_dir, f"{nid}.alphamaxgeom.sketch")
        for nid in neighbor_ids
        if os.path.exists(os.path.join(sketch_dir, f"{nid}.alphamaxgeom.sketch"))
    }
    if not ref_paths:
        return None
    ...
    try:
        ...
        for metric in ("jaccard", "containment"):
            ...
            # For AlphaMaxGeomHash, intersection and union are always 0;
...
def parse_args():
    p = argparse.ArgumentParser(
        description="AlphaMaxGeomHash pairwise similarity for GTDB candidate pairs.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--sketch-dir", required=True,
                   help="Directory of *.alphamaxgeom.sketch files "
                        "(from 03_alphamaxgeom_sketch.sh)")
...
def main():
    ...
    if not pairs:
        log.error(
            "No pairs to process. Run 03_alphamaxgeom_sketch.sh first "
    ...
    sketch_dir_size_bytes = sum(
        p.stat().st_size for p in Path(args.sketch_dir).glob("*.alphamaxgeom.sketch")
    )

    stats = {
        "script":                   "04_alphamaxgeom_pairwise.py",
    ...
    print(f"  python3 {_SCRIPT_DIR}/05_sanity_check.py \\")
