...
Usage:
  # Test run (after TEST_N=200 bash 05_alphamaxgeom_sketch.sh):
  python3 08_alphamaxgeom_pairwise.py \\
...
def parse_args():
    ...
    p.add_argument("--sketch-dir", required=True,
                   help="Directory of *.alphamaxgeom.sketch files "
                        "(from 05_alphamaxgeom_sketch.sh)")
...
def main():
    ...
    if not pairs:
        log.error(
            "No pairs to process. Run 05_alphamaxgeom_sketch.sh first "
    ...
    stats = {
        "script":                   "08_alphamaxgeom_pairwise.py",
