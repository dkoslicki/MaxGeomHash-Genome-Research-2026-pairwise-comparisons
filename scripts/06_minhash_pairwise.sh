#!/usr/bin/env bash
# ==============================================================================
# 06_minhash_pairwise.sh
#
# Wrapper: compute MinHash pairwise similarities for all genome pairs that
...
# Run 03_minhash_sketch.sh first (optionally with TEST_N to sketch a subset;
# this script will then compute only the pairs within that subset).
#
# Full run:    bash 06_minhash_pairwise.sh
# After test:  bash 06_minhash_pairwise.sh   (same command — adapts automatically)
# ==============================================================================
set -euo pipefail
...
python3 "${BASE_DIR}/scripts/06_minhash_pairwise.py" \
