#!/usr/bin/env python3
"""
07_fracminhash_pairwise.py
===========================
Compute pairwise Jaccard similarity and containment for candidate GTDB genome
pairs using FracMinHash sketches produced by the kmer-sketch binary
(--algo fracminhash, scale=0.001).
...
Usage:
  # Test run (after TEST_N=200 bash 04_fracminhash_sketch.sh):
  python3 07_fracminhash_pairwise.py \\
      --sketch-dir /scratch/.../fracminhash_sketches \\
      --candidates /scratch/.../gtdb_pairwise_containment.csv \\
      --output     /scratch/.../fracminhash_pairwise \\
      --cores      192
...
def _run_filter_for_query(args: tuple):
    ...
    worker_tmp = tempfile.mkdtemp(prefix=f"fmh_{query_id[:12]}_", dir=tmp_root)
    try:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="FracMinHash (kmer-sketch) pairwise similarity for GTDB candidate pairs.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--sketch-dir", required=True,
                   help="Directory of *.fracminhash.sketch files "
                        "(from 04_fracminhash_sketch.sh)")
...
def main():
    ...
    if not pairs:
        log.error(
            "No pairs to process. Run 04_fracminhash_sketch.sh first "
    ...
    tmp_root = tempfile.mkdtemp(prefix="fmh_pairwise_tmp_", dir=str(out_dir))
    ...
    stats = {
        "script":                   "07_fracminhash_pairwise.py",
        "algo":                     "fracminhash",
    ...
    print(f"      --fracminhash-pairwise {out_dir} \\")
